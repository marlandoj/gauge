...
import { classify as previous } from './classifier-v2.js';
...
export const frozenFiles = ['src/classifier.ts', 'src/assess.ts', 'src/receipts.ts', 'scripts/gauge.ts',
  'integrations/task-assessment-observer.ts', 'evaluation/v3/run.ts', 'evaluation/v3/classifier-v2.ts',
  'evaluation/metrics.ts', 'evaluation/review.ts', 'evaluation/PROTOCOL.md', 'evaluation/v3/PROTOCOL.md',
  'evaluation/development.jsonl', 'evaluation/development-reviewed.json', 'evaluation/review-packet.json',
  'evaluation/v2/cohort.json', 'evaluation/v2/label-approval.json', 'evaluation/v3/measure.ts'];
...
function baselineDigests() {
  const local = process.env.GAUGE_TEST_LOCAL_BASELINE, swarm = process.env.GAUGE_TEST_SWARM_ROOT;
  if (!local || !swarm || !isAbsolute(local) || !isAbsolute(swarm)) throw Error('Absolute baseline paths required');
  const files = [local, resolve(local, '../../data/weights.json'), resolve(swarm, 'src/selector/executor-selector.ts')];
  return Object.fromEntries(files.map(path => [path, hash(readFileSync(path))]));
}
export function assertAssessmentParity(candidate: ReturnType<typeof classify>, report: Awaited<ReturnType<typeof assess>>) {
  if (report.proposed_tier !== candidate.tier || report.abstained !== candidate.abstained
    || (candidate.tier !== null && !report.proposed_route)) throw Error('Production parity failure');
}
...
export function reviewedDevelopment(): Row[] {
  ...
  const raw = readFileSync(resolve(root, 'evaluation/v2/cohort.json'), 'utf8');
  const approval = JSON.parse(readFileSync(resolve(root, 'evaluation/v2/label-approval.json'), 'utf8'));
  const prior = validateRows(JSON.parse(raw));
  if (approval.human_reviewed !== true || approval.source_sha256['evaluation/v2/cohort.json'] !== hash(raw)
    || prior.length !== 30 || approval.cases.length !== prior.length) throw Error('Prior approval mismatch');
  for (const row of prior) {
    const matches = approval.cases.filter((c: {id: string}) => c.id === row.id);
    const attestation = matches[0];
    if (matches.length !== 1 || attestation.status !== 'accepted' || attestation.approved_tier !== row.tier
      || attestation.case_sha256 !== hash(JSON.stringify([row.id, row.group_id, row.task_text, row.tier, row.rationale])))
      throw Error('Prior case approval mismatch');
  }
  return [...validateRows(snapshot.cases), ...prior];
}
export function validateDiagnostic(value: unknown): Row[] {
  const rows = validateRows(value);
  if (rows.length !== 40 || new Set(rows.map(r => r.group_id)).size !== 40
    || TIERS.some(t => rows.filter(r => r.tier === t).length !== 8)
    || rows.some(r => typeof r.rationale !== 'string' || !r.rationale.trim())) throw Error('Diagnostic balance or rationale mismatch');
  return rows;
}
async function main() {
  ...
  if (command === 'freeze') {
    writeFileSync(output, JSON.stringify({ version: VERSION, frozen_at: new Date().toISOString(), files: sourceDigests(), baselines: baselineDigests() }, null, 2) + '\n', { flag: 'wx' });
    return;
  }
  ...
  if (command === 'diagnostic') {
    ...
    if (JSON.stringify(frozen.baselines) !== JSON.stringify(baselineDigests())) throw Error('Frozen baseline mismatch');
    ...
    rows = validateDiagnostic(JSON.parse(raw.toString()));
  ...
  for (const row of rows) {
    ...
    const report = await assess({ task_text: row.task_text, harness: 'codex', current_tier: swarmTier }, 'gauge-synthetic-eval-v3', resolver);
    assertAssessmentParity(candidate, report);
  ...
  const result = { version: VERSION, generated_at: new Date().toISOString(), kind: command, dataset_sha256: datasetHash,
    ...
    development_cohorts: command === 'development' ? Object.fromEntries(Object.entries(predictions).map(([k, v]) => [k, { approved_d: summarize(v.slice(0, 25)), reviewed_h: summarize(v.slice(25, approved.length)), original: summarize(v.slice(approved.length)) }])) : null,
    predictions: details };
