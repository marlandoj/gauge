...
const home = mkdtempSync(join(tmpdir(), 'zo-task-gauge-v3-latency-'));
...
const receiptPath = join(home, 'assessments.jsonl');
const rows = existsSync(receiptPath) ? readFileSync(receiptPath, 'utf8').trim().split('\n').filter(Boolean).map(line => JSON.parse(line)) : [];
assertFrozen(freeze.files);
